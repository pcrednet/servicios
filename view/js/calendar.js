...
(function($) {
    ...
    Calendar.prototype._render = function() {
        ...
        // Small hack to pass the percentage in of each cell. Must be a better way to do this
    ...
    Calendar.prototype._calculate_hour_minutes = function(data, width_percentage) {
        ...
        // data.hour_split = Math.min(data.in_hour, 1);

        if(((data.in_hour >= 1) && (data.in_hour % 1 != 0)) || ((data.in_hour < 1) && (1440 / time_split % 1 != 0))) {
            $.error(this.locale.error_timedevide);
        }

        var time = this.get_start_end_day_time();

        data.hours = (parseInt(time.end_time[0]) - parseInt(time.start_time[0])) ;// * data.hour_split;
        var lines = data.hours * data.in_hour; // data.hours * data.in_hour  - parseInt(time_start[1]) / time_split;
...
//
            ...
            var lines_left = Math.abs(lines - event.top);
